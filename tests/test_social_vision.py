...
class SocialVisionTests(unittest.IsolatedAsyncioTestCase):
    async def test_temporal_interaction_state_survives_the_next_window(self):
            ...
            [
                {
                    ...
                    "state_update": {
                        "scene_summary": "访客站在机器人前方",
                        "interaction_state": "机器人已经回应过这次挥手",
                    },
                },
                {
                    "gesture": "none",
        ...
        first = await agent.decide(
            [camera_frame(1), camera_frame(2)], state, [WaveSkill()]
        )
        second = await agent.decide(
            [camera_frame(3), camera_frame(4)], state, [WaveSkill()]
        )

        self.assertEqual(first.skill, "wave")
        self.assertEqual(second.action, "ignore")
        self.assertEqual(
            agent.temporal_state.interaction_state,
            "机器人已经回应过这次挥手",
        )
        self.assertIn("机器人已经回应过这次挥手", invoker.calls[1][1])
